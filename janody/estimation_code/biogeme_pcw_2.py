...
from biogeme.expressions import Beta, exp, bioMax, log
...
'activity_parameters' : ['late', 'early', 'short', 'constant'],
'travel_parameters' : ['travel_time', 'constant'],
...
B['act'] = Beta('act', 0.1, None, None, 0)
B['scale'] = Beta('scale', 200, None, None, 1)
B['walk:travel_time'] = Beta('walk:travel_time', 0, None, None, 1)
B['walk:constant'] = Beta('walk:constant', 0, None, None, 1)

for j in range(1,n_alt+1):
    for m in var_dict['travel_types']:
        mode_participation = database.DefineVariable(f'{m}_participation_{j}', 1*(database.variables[f'{m}:travel_time_{j}']>0))
...
def Vi(j):
    ...
    for at in var_dict['activity_types']:
        ...
        Vi+= database.variables[f'{at}:participation_{j}'] * (
                ...
                B[f'{at}:early']*early +
                B[f'{at}:short']*short +
                B['act']*avg_dur[at]*log(duration / t_0)
                #bioMax(0, B['act']*avg_dur[at]*log(duration / t_0))

                )

    for m in var_dict['travel_types']:
        Vi+= database.variables[f'{m}:participation_{j}'] * (
        B[f'{m}:constant'] +
        B[f'{m}:travel_time']*database.variables[f'{m}:travel_time_{j}']
        )
...
biogeme.modelName = f'abm_mnl_pcw_random_w_constants'
